...
#define Feed_the_joydog	Joydog = 0
#define Find_the_joydog (Joydog > 200) ? 1 : 0
uint16_t Joydog = 0;
...
FrameXbox360GLIDERData frame360gliderrec;
	...
	if (datatemp == 0x37)
	{
		...
		if (rec_data_360[3] == 0x00)
		{
			...
			if (OperateMode == 1)
			{
				...
				frame360djrec.dj_vol =f2b.f *3.0;
		...
		else if (rec_data_360[3] == 0x01)
		{
			if (OperateMode == 1)
			{
				...
				/* ESC control signal range limiter */
				...
				TxMessage.Data[1] = (3000 - frame360mtrec.motor_l) & 0xff;
				TxMessage.Data[2] = (3000 - frame360mtrec.motor_l) >> 8;
				TxMessage.Data[3] = (3000 - frame360mtrec.motor_r) & 0xff;
				TxMessage.Data[4] = (3000 - frame360mtrec.motor_r) >> 8;
				CAN_Transmit(CAN1, &TxMessage);
				/* add a func to turn via tail */
				TxMessage.ExtId = 0X1330;
				TxMessage.IDE=CAN_ID_EXT;
				TxMessage.RTR=CAN_RTR_DATA;
				TxMessage.DLC = 7;
...
//				{frame360djrec.dj_dseta = 0;}
				...
				{
						/* conver the diff data to tail angle */
		...
		else if (rec_data_360[3] == 0x03)
		{
			/* Glider control parameter set via 360 */
			memcpy(&frame360gliderrec, &rec_data_360[4], sizeof(frame360gliderrec));
			FlagOfBoomClose = 0;
			FlagOfStepClose = 0;
			BarycenterRatio = (float)frame360gliderrec.st_position / 100;
			GoPosition = frame360gliderrec.boom_position;
		}
		else if (rec_data_360[3] == 0x02)
		{
			/* Switch man or machine control mode */
...
void FindTheJoydog(void)
{
	...
	if (Find_the_joydog)
	{
		...
		TxMessage.DLC = 5;
		TxMessage.Data[0] = 0x01;
		TxMessage.Data[1] = zero_data & 0xff;
		TxMessage.Data[2] = zero_data >> 8;
		TxMessage.Data[3] = zero_data & 0xff;
		TxMessage.Data[4] = zero_data >> 8;
		CAN_Transmit(CAN1, &TxMessage);
		TxMessage.ExtId = 0X1330;
		TxMessage.IDE=CAN_ID_EXT;
		TxMessage.RTR=CAN_RTR_DATA;
...
//	DistanceIR = 51;
	...
	temp16 = TIM2->CNT;
	i++, databuf[i] = (uint8_t)((float)(temp16 - 5000) / 100.0);
	i++, databuf[i] = (0 != ((uint16_t)LeakageData[0] % 2000)) * (0 != ((uint16_t)LeakageData[1] % 2000)) *
	(0 != ((uint16_t)LeakageData[2] % 2000)) * (0 != ((uint16_t)LeakageData[3] % 2000));
	i++, databuf[i] = 0xff;
	temp8 = databuf[0];
	for (j = 1; j <= i; j++)
	{
...
void PythonDataStructInit(void)
{
			...
			framepythondata.sw_st = 1;
			framepythondata.sw_boom = 0;
}
...
void Python_Data_Of_SW_Process(void)
{
	uint8_t index, i;
	uint16_t _4th, _3th, _2nd, _1st;
	CanTxMsg TxMessage;
	...
	_2nd = (rec_char_data[index + 2] - '0') *   10;		//USART1_SendByte(rec_char_data[6]);
	_1st = (rec_char_data[index + 3] - '0');					//USART1_SendByte(rec_char_data[7]);
	framepythondata.sw_st = (int)((_4th + _3th + _2nd + _1st) - 2000);		//USART1_SendByte('X');		USART1_SendByte(' ');
	LV_EN_1 = framepythondata.sw_st;
	...
	_2nd = (rec_char_data[index + 2] - '0')*   10;		//USART1_SendByte(rec_char_data[10]);
	_1st = (rec_char_data[index + 3] - '0');					//USART1_SendByte(rec_char_data[11]);
	framepythondata.sw_boom = (int)(_4th + _3th + _2nd + _1st) - 2000;		//USART1_SendByte('Y');		USART1_SendByte(' ');
	MO_EN_1 = framepythondata.sw_boom;
