...
作    者:何宇帆(2016-09-20)
**************************************************************************/
uint8_t print_xor_byte = 0;
uint8_t print_rec_byte = 0;
unsigned char XORCheck(void)
{
	unsigned char i = 0;
	...
	print_xor_byte = xor_byte;
	print_rec_byte = rec_data[rec_data[2] - 2];
...
//				while(uart1sendflag);
				...
				RaspiDataSend[RDS_EC]    = RS485_RECEIVED_DATA.ec_ecdata;
				RaspiDataSend[RDS_L1]    = LeakageData[0];
				RaspiDataSend[RDS_L2]    = LeakageData[1];
				RaspiDataSend[RDS_L3]    = LeakageData[2];
				RaspiDataSend[RDS_L4]    = LeakageData[3];
				RaspiDataSend[RDS_BATV]  = BatIV[0];
				RaspiDataSend[RDS_BATI]  = BatIV[1];
				RaspiDataSend[RDS_BOOM]  = (float)((DistanceIR - 30.0) / 80.0 * 100);
				tim2_cnt_temp = TIM2->CNT;
				RaspiDataSend[RDS_POSI]  = ((float)(tim2_cnt_temp - 5000) / 20.0);
			...
			case 0x37:
				...
				if (OperateMode == 0)
				{
					...
					TxMessage.Data[1] = (3000 - framepushrec.motor_l) & 0xff;
					TxMessage.Data[2] = (3000 - framepushrec.motor_l) >> 8;
					TxMessage.Data[3] = (3000 - framepushrec.motor_r) & 0xff;
					TxMessage.Data[4] = (3000 - framepushrec.motor_r) >> 8;
					CAN_Transmit(CAN1, &TxMessage);
				}
				break;
			/* GPS数据传输 */
			case 0x36:
				/* 存储GPS信息到framehgpsrec结构体中,消耗:上传至遥控器 */
//				memcpy(&framehgpsrec, &rec_data[4], sizeof(framehgpsrec));
				lengty = sizeof(framehgpsrec);
				f2b.b[0] = rec_data[4];
				f2b.b[1] = rec_data[5];
				f2b.b[2] = rec_data[6];
				f2b.b[3] = rec_data[7];
				framehgpsrec.gps_vel = 44;
				
				f2b.b[0] = rec_data[8];
				f2b.b[1] = rec_data[9];
				f2b.b[2] = rec_data[10];
				f2b.b[3] = rec_data[11];
				framehgpsrec.gps_dir = f2b.f;

				for (i = 0; i < 8; i++)
				{
					f2b.b[i] = rec_data[12 + i];
				}
				//framehgpsrec.gps_longitude = f2b.f;
				for (i = 0; i < 8; i++)
				{
					f2b.b[i] = rec_data[20 + i];
				}
				framehgpsrec.gps_latitude = f2b.f;
				break;
			/* Function flag */
			case 0x35:
